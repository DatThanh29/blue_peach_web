...
export default function AccountPage() {
  ...
  async function handleLogout() {
    try {
      ...
      console.error("[AccountPage] signOut failed:", error);
      setToast({
            ...
            <div className="space-y-6">
              <div className="rounded-[28px] border border-black/8 bg-[linear-gradient(180deg,#f7f1e7_0%,#f2eadf_100%)] p-6 md:p-8">
                ...
                <p className="mt-4 max-w-[760px] text-sm leading-7 text-black/65">
                  Đây là khu vực thông tin chung của tài khoản. Bạn có thể xem nhanh
                  các dữ liệu cơ bản của mình trước khi tiếp tục chỉnh sửa hồ sơ,
                  quản lý địa chỉ nhận hàng, theo dõi đơn hàng và các tính năng mở rộng sau này.
                </p>

                <div className="mt-8 flex flex-wrap gap-3">
                  <Link href="/account/profile" className="bp-btn bp-btn--solid border-black">
                    Chỉnh sửa hồ sơ
                  </Link>
                  <button
                    type="button"
              ...
              <div className="rounded-[28px] border border-black/8 bg-white/90 p-6 shadow-[0_16px_50px_rgba(0,0,0,0.04)] md:p-8">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <Link href="/account/profile" className="bp-btn bp-btn--ghost !px-4 !py-2">
                    Chỉnh sửa
                  </Link>
                </div>

                <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  <InfoTile label="Họ và tên" value={profile?.full_name || "Chưa cập nhật"} />
                  <InfoTile label="Email" value={user?.email || profile?.email || "Chưa có"} />
                  <InfoTile label="Số điện thoại" value={profile?.phone || "Chưa cập nhật"} />
                  <InfoTile label="Vai trò" value={profile?.role || "customer"} />
                  <InfoTile label="Ngày tham gia" value={memberSince} />
                  <InfoTile label="Xác minh email" value="Đã xác minh" />
                </div>
              </div>
            </div>
          </div>
...
function InfoTile({
  ...
  return (
    <div className="rounded-[22px] border border-black/8 bg-[#fbfaf7] px-4 py-4">
